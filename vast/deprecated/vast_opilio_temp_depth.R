### Univariate Immature Opilio with Temperature and Depth covariates ###
...
fp = paste0(getwd(),'/vast/output/opilio_temp_depth/')
dir.create(fp)

model_directory <- paste0(getwd(),'/vast/output/opilio_temp_depth/model/')
...
dat <- dat_opilio %>%
  mutate(spp=factor(spp,levels=c("Opilio immature","Opilio spawner"))) %>% 
  ...
  dplyr::select(spp,year,lon,lat,area_km2,abun,vessel)
...
Species_set = c("Opilio immature","Opilio spawner")
