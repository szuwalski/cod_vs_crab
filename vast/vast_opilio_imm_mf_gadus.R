...
fp = paste0(getwd(),'/vast/output/opilio_gadus_cov/')
dir.create(fp)

model_directory <- paste0(getwd(),'/vast/output/opilio_gadus_cov/model/')
dir.create(model_directory)

# filter for immature or mature female crabs, summarise total by station/year, and row-bind cod data
dat_opilio <- opi_dat_long %>% 
  ...
  group_by(spp,towid,year,lon,lat,area_km2) %>% 
...
Species_set = c("Gadus macrocephalus","Opilio immature","Opilio spawner")

# for this analysis we only use cod data
...
# we pick model "4" here for the observation model for encounter rates, adjusting to delta-model for years with 0 or 100% encounter rate
ObsModel = c(1, 4)
